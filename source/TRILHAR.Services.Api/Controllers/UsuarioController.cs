...
namespace TRILHAR.Services.Api.Controllers
{
    ...
    public class UsuarioController : BaseApiController
    {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(ProblemDetails))]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(ProblemDetails))]
        public async Task<IActionResult> CreateUsuarioAsync([FromBody] UsuarioInput registro)
        {
            if (!ModelState.IsValid) return CustomResponse(ModelState);

            _logger.LogInformation("Criando novo usuário: {@Usuario}", registro);
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(ProblemDetails))]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(ProblemDetails))]
        public async Task<IActionResult> UpdateUsuarioAsync(int id, [FromBody] UsuarioInput input)
        {
            if (!ModelState.IsValid) return CustomResponse(ModelState);
            ...
            if (reg == null)
            {
                _logger.LogWarning("Tentativa de atualização para usuario ID {Id}, mas não encontrado.", id);
            ...
            _logger.LogInformation("Atualizando usuário ID {Id}: {@Input}", id, input);
